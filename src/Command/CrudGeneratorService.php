...
class CrudGeneratorService
{
    ...
    public function generateCrud($db, $entity, $table)
    {
        $this->entity = $entity;
        $this->table = $table;
        $this->entityUpper = ucfirst($this->entity);
        $table = new CrudGeneratorEntity();
        $table->getParamsAndFields($db, $this->table);
        $this->updateRoutes();
    ...
    private function getBaseInsertQueryFunction($table)
    {
        ...
        return '$query = \'INSERT INTO `'.$this->table.'` ('.$table->list1.') VALUES ('.$table->list2.')\';
    ...
    private function getBaseUpdateQueryFunction($table)
    {
        // Get Base Query For Update Function and return this Mock Code...
        return $table->list5.'

        $query = \'UPDATE `'.$this->table.'` SET '.$table->list4.' WHERE `id` = :id\';
        $statement = $this->getDb()->prepare($query);
        '.$table->list3.'
        $statement->execute();

        return $this->checkAndGet((int) $'.$this->table.'->id);';
    }
...
';
    ...
    private function replaceFileContentTable($target)
    {
        $content1 = file_get_contents($target);
        $content2 = preg_replace("/".'Objectbase'."/", $this->entityUpper, $content1);
        $content3 = preg_replace("/".'objectbase'."/", $this->table, $content2);
        file_put_contents($target, $content3);
    }
    ...
    private function updateRepository2()
    {
        ...
        copy($source, $target);
        $this->replaceFileContentTable($target);
    }

    private function updateRepository3($table)
    {
        $target = __DIR__ . '/../../../../../src/Repository/' . $this->entityUpper . 'Repository.php';

        $entityRepository = file_get_contents($target);
        $repositoryData = preg_replace("/".'#createFunction'."/", $this->getBaseInsertQueryFunction($table), $entityRepository);
        file_put_contents($target, $repositoryData);

        $entityRepositoryUpdate = file_get_contents($target);
        $repositoryDataUpdate = preg_replace("/".'#updateFunction'."/", $this->getBaseUpdateQueryFunction($table), $entityRepositoryUpdate);
